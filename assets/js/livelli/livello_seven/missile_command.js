...
var missileCommand = (function() {
  ...
  function PlayerMissile( source, endX, endY ) {
    ...
    var scale = (function() {
      ...
      distanzaX = xDistance;
      distanzaY = yDistance;
      distance = calcola_esito(xDistance,yDistance);
      var  distancePerFrame = ( source === 1 ) ? 20 : 12;  
...
function getTest(){
 ...
 if( distance < (Math.sqrt( Math.pow(distanzaX, 2) + Math.pow(distanzaY, 2) ) + 1) && 
   distance > (Math.sqrt( Math.pow(distanzaX, 2) + Math.pow(distanzaY, 2) ) - 1) )
