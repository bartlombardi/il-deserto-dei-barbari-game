...
var missileCommand = (function() {
  ...
  function EnemyMissile( targets ) {
    ...
    /*fix[0]=-100;
    fix[1]=-100;*/
    fix_four = calcola_velocita();
    framesToTarget = ( 650 - 30 * level ) / fix_four; //offspeed
  ...
  var setupListeners = function() {
    $( '.container' ).one( 'click', function() {
      ...
      $( '.container' ).on( 'click', function( event ) {
        ...
        fix = playerShoot( event.pageX - this.offsetLeft, 
...
function getTest(testo){
  var count = (testo.match(/while\(/g) || []).length;
  count += (testo.match(/for\(/g) || []).length;
  console.log(count);
  if((fix_four ==  0.8) && ( count <= 1))
    return true;
  else
    return false;
