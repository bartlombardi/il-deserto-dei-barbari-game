...
var missileCommand = (function() {
  ...
  /*  var matrix = [];
    ...
    matrix = imposta_posizioni(80,300);
    cities.push( new City( matrix[0][0], 430 ) );
    cities.push( new City( matrix[0][1], 430 ) );
    cities.push( new City( matrix[0][2], 430 ) );
    cities.push( new City( matrix[1][0], 430 ) );
    cities.push( new City( matrix[1][1], 430 ) );
    cities.push( new City( matrix[1][2], 430 ) );
  ...
  var drawSyntaxError = function() {
    ...
    ctx.fillText( 'Errore di Sintassi', 90, 240 );
    ctx.fillStyle = 'yellow';
    ctx.font = 'bold 26px arial';
    //ctx.fillText( 'Final Score: ' + score, 80, 20 );
    //ctx.fillText( 'CLICK TO PLAY NEW GAME', 80, 458 );
  ...
  //$.import_js('/assets/js/livello_one.js');
  ...
  function EnemyMissile( targets ) {
    ...
    /*fix[0]=-100;
    fix[1]=-100;*/
    framesToTarget = ( 650 - 30 * level ) / 0.8; //offspeed
  ...
  var loadEnemyMissiles = function(targets){
  		enemyMissiles.push( new EnemyMissile(targets) );
   }
   var loadMissiles = function(source,x,y){
      playerMissiles.push( new PlayerMissile( source, x, y ) );
   }
 // Attach event Listeners to handle the player's input
  ...
  return {
    ...
    loadEnemyMissiles: loadEnemyMissiles,
    viableTargets: viableTargets,
    get_level: get_level,
    loadMissiles: loadMissiles,
    whichAntiMissileBattery: whichAntiMissileBattery,
    drawSyntaxError: drawSyntaxError,
    endGame: endGame,
    stopLevel: stopLevel,
    stop: stop
...
function getTest(){
 ...
 return ((matrix != null) && (matrix[0][0] == 80) && (matrix[0][1] == 130) && (matrix[0][2] == 180) 
                    && (matrix[1][0] == 300) && (matrix[1][1] == 350)&& (matrix[1][2] == 400));

}
$( document ).ready( function() {
 
if( missileCommand.stop==false){
